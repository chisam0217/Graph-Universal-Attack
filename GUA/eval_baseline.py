...
if args.evaluate_mode == "universal":
    ...
    all_entropy = []
    for i in range(2,3):
        perturb = np.array([float(line.rstrip('\n')) for line in open('./vision4_result/{1}_xi{2}_epoch100/perturbation_{1}_{0}.txt'.format(i, args.dataset, args.radius))])
        perturb = np.where(perturb>0.5, 1, 0)
...
elif args.evaluate_mode == "victim_attack":
    #the perturbation times of our universal perturbation
    perturb_time = 6
    fool_res = []
#     p_times = []
...
#         for i in range(1):
    ...
    for i in range(1,8):
        fool_res = []
        for j in range(8):
            perturb = np.array([float(line.rstrip('\n')) for line in open('./vision4_result/{1}_xi{2}_epoch100/perturbation_{1}_2.txt'.format(i, args.dataset, args.radius))])
            perturb = np.where(perturb>0.5, 1, 0)
            pt = np.where(perturb>0)[0]
