...
def universal_attack(attack_epoch, max_epoch):
	...
	#             print ('iter', iter)
				...
				if iter < max_iter_df-1:
					...
					print ('the distance of perturbation is', np.linalg.norm(v.flatten(1)))
					print ('the sum of the perturbation', sum(v))
	...
	#     v = np.random.rand(tmp_adj.shape[0])
		...
		if fooling_rate >= cur_foolingrate:
			...
			with open('./version4_itr30/{1}_xi{2}_epoch100/perturbation_{1}_{0}.txt'.format(attack_epoch, args.dataset, args.radius), 'w') as f:
				for i in v:
					f.write(str(i) + '\n')
#################
