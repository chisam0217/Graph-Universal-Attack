...
def load_data(dataset_str):
    ...
    G = nx.from_scipy_sparse_matrix(adj.tocoo(), create_using=nx.Graph())
   
    #for citeseer
    if dataset_str == 'citeseer':
        add_lab = np.full((len(G) - labels.shape[0]), 0)
...
def load_polblogs_data(filepath = '../data/data/polblogs.npz'):
    ...
    G = nx.from_scipy_sparse_matrix(_A_obs.tocoo(), create_using=nx.Graph())
    return G, features, labels, split_train, split_unlabeled
...
def main():
    G, features, labels, idx_train, idx_val, idx_test = load_data("cora")
    # G = nx.Graph()

    # adj = adj.tocoo()
    # edge_num = len(adj.row)
    # node_num = max(adj.row)
    # print (node_num)
    # for i in range(node_num):
    #     G.add_node(i)
    # for i in range(len(adj.row)):
    #     G.add_edge(adj.row[i], adj.col[i])
    print ('nodes', G.nodes)
    print ('G', G.edges)
    # print ('G', G.neighbors(1))
    for i in G.neighbors(0):
        print (i)
    if 633 in G.neighbors(0):
        print ('found')
    # print (adj)
    # print (type(adj))
